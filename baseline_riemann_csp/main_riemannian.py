...
class Riemannian_Model:
	...
	def __init__(self,num_classes=4):
		...
		self.svm_c = 0.01 # for linear 0.1 (inverse),
		...
		self.bw = np.array([4]) # bandwidth of filtered signals 
		...
		time_windows_flt = np.array([[0,3]])*self.fs
		self.time_windows = time_windows_flt.astype(int)
		# restrict time windows and frequency bands 
		#self.time_windows = self.time_windows# use only largest timewindow
		...
		self.riem_opt = "Riemann_Euclid" # {"Riemann","Riemann_Euclid","Whitened_Euclid","No_Adaptation"}
		# time measurements 
		self.train_time = 0
		self.train_trials = 0
		self.eval_time = 0
	...
	def load_data(self):		
		if self.crossvalidation:
			npzfile = np.load(self.data_path+f'{self.num_classes}class.npz')
			data, label = npzfile['X_Train'], npzfile['y_Train']
...
def main():
	...
	for clas in [2,3,4]:
		print("{:}-class MI".format(clas))
		model = Riemannian_Model(num_classes=clas)
		...
		for model.subject in range(model.NO_subjects):
