    ...
    for train_global, test_global in kf_global.split(subjects):
        for sub_idx in test_global:
            ...
            for train_sub, test_sub in kf_subject.split(X_sub, y_sub):
                ...
                # Save metrics
                train_accu_str = f'{results_dir}/stats/train_accu_v01_c_{num_classes}_sub_{sub_str}_fold_{sub_split_ctr}.csv'
                valid_accu_str = f'{results_dir}/stats/valid_accu_v01_c_{num_classes}_sub_{sub_str}_fold_{sub_split_ctr}.csv'
                train_loss_str = f'{results_dir}/stats/train_loss_v01_c_{num_classes}_sub_{sub_str}_fold_{sub_split_ctr}.csv'
                valid_loss_str = f'{results_dir}/stats/valid_loss_v01_c_{num_classes}_sub_{sub_str}_fold_{sub_split_ctr}.csv'
...
for num_classes in num_classes_list:
    ...
    for subject in subjects:
        ...
        for sub_split_ctr in range(0,4):
            # Save metrics
            train_accu_step = np.loadtxt(f'{results_dir}/stats/train_accu_v01_c_{num_classes}_sub_{sub_str}_fold_{sub_split_ctr}.csv')
            valid_accu_step = np.loadtxt(f'{results_dir}/stats/valid_accu_v01_c_{num_classes}_sub_{sub_str}_fold_{sub_split_ctr}.csv')
            train_loss_step = np.loadtxt(f'{results_dir}/stats/train_loss_v01_c_{num_classes}_sub_{sub_str}_fold_{sub_split_ctr}.csv')
            valid_loss_step = np.loadtxt(f'{results_dir}/stats/valid_loss_v01_c_{num_classes}_sub_{sub_str}_fold_{sub_split_ctr}.csv')
        ...
        np.savetxt(f'{results_dir}/stats/{num_classes}_class/train_accu_v01_c_{num_classes}_sub_{sub_str}_avg.csv', train_accu)
        np.savetxt(f'{results_dir}/stats/{num_classes}_class/valid_accu_v01_c_{num_classes}_sub_{sub_str}_avg.csv', valid_accu)
        np.savetxt(f'{results_dir}/stats/{num_classes}_class/train_loss_v01_c_{num_classes}_sub_{sub_str}_avg.csv', train_loss)
        np.savetxt(f'{results_dir}/stats/{num_classes}_class/valid_loss_v01_c_{num_classes}_sub_{sub_str}_avg.csv', valid_loss)
for num_classes in num_classes_list:
    train_accu = np.zeros(n_epochs+1)
    valid_accu = np.zeros(n_epochs+1)
    train_loss = np.zeros(n_epochs+1)
    ...
    for subject in subjects:
        sub_str = '{0:03d}'.format(subject)
        train_accu_step = np.loadtxt(f'{results_dir}/stats/{num_classes}_class/train_accu_v01_c_{num_classes}_sub_{sub_str}_avg.csv')
        valid_accu_step = np.loadtxt(f'{results_dir}/stats/{num_classes}_class/valid_accu_v01_c_{num_classes}_sub_{sub_str}_avg.csv')
        train_loss_step = np.loadtxt(f'{results_dir}/stats/{num_classes}_class/train_loss_v01_c_{num_classes}_sub_{sub_str}_avg.csv')
        valid_loss_step = np.loadtxt(f'{results_dir}/stats/{num_classes}_class/valid_loss_v01_c_{num_classes}_sub_{sub_str}_avg.csv')
    ...
    np.savetxt(f'{results_dir}/stats/avg/train_accu_v01_c_{num_classes}_ss_avg.csv', train_accu)
    np.savetxt(f'{results_dir}/stats/avg/valid_accu_v01_c_{num_classes}_ss_avg.csv', valid_accu)
    np.savetxt(f'{results_dir}/stats/avg/train_loss_v01_c_{num_classes}_ss_avg.csv', train_loss)
    np.savetxt(f'{results_dir}/stats/avg/valid_loss_v01_c_{num_classes}_ss_avg.csv', valid_loss)
