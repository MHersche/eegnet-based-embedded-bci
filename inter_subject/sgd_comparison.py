...
sgd0 =  keras.optimizers.SGD(lr=0.01, momentum=0.0, nesterov=False)
sgd1 =  keras.optimizers.SGD(lr=0.001, momentum=0.0, nesterov=False)
sgd2 =  keras.optimizers.SGD(lr=0.01, momentum=0.8, nesterov=False)
sgd3 =  keras.optimizers.SGD(lr=0.001, momentum=0.8, nesterov=False)
sgd4 =  keras.optimizers.SGD(lr=0.01, momentum=0.8, nesterov=True)
sgd5 =  keras.optimizers.SGD(lr=0.001, momentum=0.8, nesterov=True)

optimizers = [sgd0, sgd1, sgd2, sgd3, sgd4, sgd5]
...
for j, optimizer in enumerate(optimizers):
    ...
    for train, test in kf.split(X_Train_real, y_Train):
        ...
        # Save metrics
        train_accu_str = f'{results_dir}/stats/train_accu_sgd{j}_s{split_ctr}.csv'
        valid_accu_str = f'{results_dir}/stats/valid_accu_sgd{j}_s{split_ctr}.csv'
        train_loss_str = f'{results_dir}/stats/train_loss_sgd{j}_s{split_ctr}.csv'
        valid_loss_str = f'{results_dir}/stats/valid_loss_sgd{j}_s{split_ctr}.csv'
    ...
    train_accu_str = f'{results_dir}/stats/train_accu_sgd{j}_avg.csv'
    valid_accu_str = f'{results_dir}/stats/valid_accu_sgd{j}_avg.csv'
    train_loss_str = f'{results_dir}/stats/train_loss_sgd{j}_avg.csv'
    valid_loss_str = f'{results_dir}/stats/valid_loss_sgd{j}_avg.csv'
